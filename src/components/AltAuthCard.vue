  ...
  >
    ...
    <template v-if="background">
      <div
        ...
        style="background: radial-gradient(circle, #f9b233 0%, transparent 70%)"
      />
      <div
        class="pointer-events-none absolute -bottom-32 -left-24 w-[28rem] h-[28rem] rounded-full opacity-25 blur-3xl"
        style="background: radial-gradient(circle, #2d519f 0%, transparent 70%)"
      />
      <svg
        class="pointer-events-none absolute bottom-0 left-0 w-full opacity-[0.07]"
      ...
      >
        <path
          fill="#ffffff"
          d="M0,160 C320,260 640,60 960,120 C1200,170 1320,240 1440,200 L1440,320 L0,320 Z"
        />
      </svg>
    </template>
    ...
    >
      <div class="bg-white dark:bg-alt-dark-surface rounded-2xl shadow-altoneo-pop border border-gray-100 dark:border-alt-dark-border p-8">
        ...
        <div class="mb-6 text-center">
          ...
          <h1
            v-if="title"
            class="text-xl font-bold text-altoneo-800 dark:text-altoneo-50"
          >
            {{ title }}
          </h1>
          <p
            v-if="subtitle"
            class="text-sm text-gray-500 dark:text-alt-dark-muted mt-1"
          >
            {{ subtitle }}
          </p>
        </div>
        ...
        <div
          v-if="$slots.footer"
          class="mt-6 text-center text-sm text-gray-500 dark:text-alt-dark-muted"
        >
          <slot name="footer" />
        </div>
      </div>
